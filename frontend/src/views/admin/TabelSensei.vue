        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h4 class="modal-title">Ubah Data Sensei</h4>
    ...
    <div class="modal" id="myModal4">
        <div class="modal-dialog">
            <div class="modal-content">
                <!-- Modal Header -->
                <div class="modal-header">
                    <h4 class="modal-title">Tambah Sensei</h4>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <!-- Modal body -->
                <div class="modal-body">
...
export default {
  ...
  methods: {
    load(){
        axios.get('http://localhost:3000/sensei')
        .then(res => {
        this.users = res.data
      }).catch ((err) => {
        console.log(err);
    ...
    add(){
        axios.post('http://localhost:3000/sensei/', this.form).then(res => {
            Swal.fire({
            position: 'center',
            icon: 'success',
    ...
    del(user){
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios.delete('http://localhost:3000/sensei/' + user.id).then(res =>{
          this.load()
          Swal.fire(
            'Di Hapus!',
            'Data telah dihapus',
    ...
    update(form){
        return axios.put('http://localhost:3000/sensei/' + form.id , {nama: this.form.nama, alamat: this.form.alamat, tmptlahir: this.form.tmptlahir, tgllahir: this.form.tgllahir, notelp: this.form.notelp, jkel: this.form.jkel, email: this.form.email, pwd: this.form.pwd, id_status: this.form.id_status  }).then(res => {
          this.load()
          Swal.fire(
            'Tersimpan!',
